...
> **Status**: Pending — Phase A ready for implementation
...
- [ ] **A.1.1** Implement `parse_args(argc, argv) -> Result<NcConfig, NcError>`:
...
- [ ] **A.1.2** Implement hostname/IP resolution in argument parsing:
...
- [ ] **A.2.1** Implement output utility functions:
...
- [ ] **A.3.1** Implement `udp_client(config: &NcConfig)`:
...
- [ ] **A.3.2** Implement client I/O loop (half-duplex, pre-poll):
...
- [ ] **A.3.3** Implement `read_line_from_stdin(buf, max_len) -> usize`:
...
- [ ] **A.4.1** Implement `udp_listen(config: &NcConfig)`:
  - `socket(AF_INET, SOCK_DGRAM, 0)` — create UDP socket
...
- [ ] **A.4.2** Implement listener receive loop:
...
- [ ] **GATE**: `nc -u 10.0.2.2 7` (QEMU SLIRP echo port) sends a line and receives the echo
- [ ] **GATE**: `nc -u -l 12345` receives UDP packets sent from the host into QEMU
- [ ] **GATE**: `-v` flag prints connection info and per-packet metadata
- [ ] **GATE**: `-w 5` exits after 5 seconds of no data
- [ ] **GATE**: `-p 54321` sets the source port correctly (visible in packet capture)
- [ ] **GATE**: Invalid arguments print usage and exit 1
- [ ] **GATE**: Hostname resolution works via kernel DNS: `nc -u gateway 53`
- [ ] **GATE**: Ctrl+C exits cleanly (no hang, no panic)
...
```
userland/src/apps/nc.rs         — All nc logic (single file for Phase A)
userland/src/bin/nc.rs          — Entry point (3 lines)
```
...
**`userland/src/apps/mod.rs`** — add `pub mod nc;`

**`userland/src/program_registry.rs`** — add entry:
```rust
ProgramSpec {
    name: b"nc",
    path: b"/bin/nc",
...
**`userland/src/bin/nc.rs`**:
```rust
#![no_std]
#![no_main]
slopos_userland::entry!(slopos_userland::apps::nc::nc_main);
```

### Build System

The `justfile` + Cargo workspace should auto-discover the new binary via `userland/src/bin/nc.rs` — verify the binary gets included in the ext2 filesystem image at `/bin/nc`.
...
| Future: raw mode | Phase 9 | ICMP echo (but that's `ping`, not `nc`) |

Phase A is implementable **right now** with zero kernel changes. That's the whole point.
