...
pub fn render_bitmap_glyph(codepoint: u8, out: &mut [u8]) -> (u16, u16) {
    ...
    let h = BITMAP_FONT_HEIGHT as usize;
    for row in 0..h {
        let byte = bitmap[row];
        for col in 0..w {
            let pixel = if (byte >> (7 - col)) & 1 != 0 { 255 } else { 0 };
            let idx = row * w + col;
            if idx < out.len() {
                out[idx] = pixel;
            }
...
pub static VGA_FONT_8X16: [u8; 4096] = [
    ...
    0x00, 0x00, 0x00, 0x00, 0x10, 0x38, 0x6C, 0xC6, 0xC6, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00, // 0x7F,
