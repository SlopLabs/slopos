#!/usr/bin/env bash
# Verify the pinned Rust std + libc forks the `slopos` sysroot is built from.
#
...
# ship unnoticed.
# The failure mode moves instead: the fork can drift from the compiler it was
# cut against, a patch can change without its pin changing, and a materialised
# sysroot can go stale or lose its registration. All four are silent — a stale
# sysroot compiles, it just compiles last week's std — so they are gated rather
# than reviewed.
#
# The four failures:
#   1. `toolchain/PIN`'s channel disagrees with `rust-toolchain.toml`.
#   2. A `toolchain/**/*.patch` file's sha256 disagrees with its PIN line (or
#      has no line, or a line names a patch that is not there).
#   3. `third_party/rust-slopos` exists but its `.slopos-stamp` is stale.
#   4. A linked `slopos` toolchain points somewhere other than
#      `third_party/rust-slopos`.
#
# Checks 3 and 4 are conditional on the thing existing, so CI that never
# materialises a sysroot still passes — the pin itself is what every checkout
# must agree on.
#
...
run_gate() {
    ...
    local rel want got count=0
    for rel in $(tp_patch_files "$root"); do
        count=$((count + 1))
        want="$(tp_pin_patch_sha "$pin" "$rel")"
        if [ -z "$want" ]; then
            fail "$rel has no \`patch_sha256=$rel:<sha256>\` line in $TP_PIN_REL"
            continue
        fi
    ...
    while read -r rel _; do
        [ -n "$rel" ] || continue
        [ -f "$root/$rel" ] || fail "$TP_PIN_REL pins $rel, which does not exist"
    done <<EOF
$(tp_pin_patches "$pin")
EOF

    # 3. A materialised sysroot must match the overlay it was built from.
    local sysroot stamp stamp_want stamp_note=""
    sysroot="$root/$TP_SYSROOT_REL"
    stamp="$sysroot/$TP_STAMP_NAME"
    stamp_want="$(tp_stamp "$root")"
    if [ -d "$sysroot" ]; then
        if [ ! -f "$stamp" ]; then
            fail "$TP_SYSROOT_REL exists with no $TP_STAMP_NAME — re-run scripts/make_slopos_sysroot.sh"
        elif [ "$(cat "$stamp")" != "$stamp_want" ]; then
            fail "$TP_SYSROOT_REL is stale: stamp $(cat "$stamp") but the overlay hashes to $stamp_want
       Re-run scripts/make_slopos_sysroot.sh — a stale sysroot compiles, it just
       compiles the previous fork."
        else
            stamp_note=", sysroot materialised"
        fi
        ...
        local created missing=""
        for rel in $(tp_patch_files "$root"); do
            local applied_in
            case "$rel" in
                "$TP_OVERLAY_REL/libc/"*) applied_in="$sysroot/$TP_LIBRARY_REL/libc" ;;
                *) applied_in="$sysroot/$TP_LIBRARY_REL" ;;
            esac
            for created in $(tp_patch_new_files "$root/$rel"); do
                [ -e "$applied_in/$created" ] || missing="$missing
         ${applied_in#"$root/"}/$created ($rel)"
            done
        done
        if [ -n "$missing" ]; then
            fail "$TP_SYSROOT_REL is stamped current but the patches did not land:$missing
       Delete $TP_SYSROOT_REL and re-run scripts/make_slopos_sysroot.sh."
        fi
    fi

    # 4. A registration that points elsewhere builds someone else's std.
    local linked link_note=""
    ...
    if [ "$bad" -ne 0 ]; then
        echo "$SELF: FAIL — the std/libc fork no longer matches its pin" >&2
        return 1
    fi

    echo "$SELF: OK — channel $pin_channel, libc $libc_version, $count patch(es), stamp $stamp_want$stamp_note$link_note"
    return 0
}
...
FIXTURE
    ...
    # A materialised sysroot in which both halves of the fork landed.
    materialise() {
    ...
    run_case good-materialised 0 "sysroot materialised, linked as" \
        "silent on a fresh sysroot stamped and linked where it belongs"
